...
const ChokaiPanel: React.FC<ChokaiPanelProps> = ({ kairanbans, missions, onReadKairanban, onJoinMission, selectedAreas, userRole, onOpenCreateMission, myCommunities, joinedMissionIds }) => {
  ...
  // Add local state to track processing IDs
  ...
  const handleRead = async (id: string, points: number) => {
    ...
    try {
      ...
      // Small delay just to prevent rapid re-click before UI updates from parent
      // Note: Usually the parent updates 'k.isRead' so this button disappears anyway
      setTimeout(() => {
